...
def EIFS_automation(src, coord):
    ...
    def img_trans(img):
        img_trans = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        # img_trans = cv2.inRange(img_trans, (120, 100, 80), (225, 190, 160))
        # img_trans = cv2.inRange(img_trans, (100, 90, 50), (225, 180, 160))
        img_trans = cv2.inRange(img_trans, (130, 120, 140), (185, 200, 200))
    ...
    cv2.imshow("figure", imgOutput)
    cv2.waitKey(0)
    plt.imshow(imgOutput)
    plt.show()
    adhesive, rest, img_trans_HSV = img_trans(imgOutput)
    plt.imshow(img_trans_HSV)
    plt.show()
    ...
    with open(dir, 'r', encoding='utf-8') as f:
        ...
        for line in lines:
            ...
            if len(tmp) > 100:
                ...
                idx += 1
    return coord
coord = np.loadtxt("./output_0.csv", delimiter=",")
